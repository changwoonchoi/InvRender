...
srgb2rgb = lambda x: np.power(x, 2.2)
    ...
    def __init__(self,
        ...
        for frame in meta['frames']:
            ...
            if split == 'train':
                ...
                image_paths.append(os.path.join(self.instance_dir, frame['file_path'] + '.png'))
        ...
        # read training images and mask images
        for path in image_paths:
            rgb = rend_util.load_rgb(path).reshape(-1, 3)
            self.rgb_images.append(torch.from_numpy(rgb).float())
            self.has_groundtruth = True
            mask_path = os.path.join(os.path.split(path)[0], 'mask.png')
            object_mask = rend_util.load_mask(mask_path)
